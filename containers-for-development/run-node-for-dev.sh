...
print_usage() {
    echo -e "${YELLOW}Usage: $0 [-e|--engine <engine>] [-i|--image-name <image_name>] [-p|--pod-name <pod_name>] [--network <network_name>] [-c|--container-name <container_name>] [-m|--mount <mount_path>] [--remove-existing|-re] [--remove-force|-rf] [-h|--help]${NC}"
    ...
    echo -e "${YELLOW}  -c, --container-name <container_name> Specify the container name${NC}"
...
echo -e "${LIGHT_BLUE}Starting node container: $CMD${NC}"
eval $CMD
COMMAND_EXIT_CODE=$?

if [ $COMMAND_EXIT_CODE -ne 0 ]; then
    echo -e "${RED}Failed to start Node container: $CONTAINER_NAME.${NC}"
    exit $COMMAND_EXIT_CODE
else
    echo -e "${GREEN}Node container $CONTAINER_NAME started successfully.${NC}"
fi