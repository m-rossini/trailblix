...
def register():
    ...
    marketing_consent_data = data.get("marketing_consent_data")
    current_app.logger.info(
        f"Registering user: {username}, marketing_consent_data: {marketing_consent_data}"
    )
    msg, code, to_return = s_register(
        username,
        password,
        display_name,
        birth_date,
...
def update_user_profile():
    ...
    marketing_consent_data = data.get("marketing_consent_data")
    current_app.logger.info(
        f"Updating user: {username}, password: {password}"
    )
    msg, code, to_return = s_update_profile(
        username,
        password,
        display_name,
        birth_date,
